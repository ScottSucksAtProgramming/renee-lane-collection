...
def test_artist_payout_calculated_correctly():
...
def test_currentTokenID_increments_after_successful_mint():
...
def test_mintArtwork_logs_from_address_correctly():
...
def test_mintArtwork_logs_to_address_correctly():
...
def test_mintArtwork_logs_tokenID_correctly():
...
def test_mintArtwork_reverts_if_no_value_sent():
...
def test_mintArtwork_reverts_when_imageNumber_is_above_range():
...
def test_mintArtwork_reverts_when_imageNumber_is_below_range():
    """Tests to see if mintArtwork reverts when imageNumber is out of range.
    Test will pass if the transaction reverts."""
    # Arrange
    deployer_account = get_account()
...
def test_mintArtwork_reverts_when_imageNumber_is_negative():
...
def test_mintArtwork_reverts_when_imageNumber_is_float():
...
def test_mintArtwork_reverts_when_imageNumber_is_string():
...
def test_mintArtwork_reverts_if_no_more_tokens_available():
...
def test_mintArtwork_reverts_if_incorrect_value_sent():
    ...
    with reverts("You didn't send the correct amount of Ether."):
        contract.mintArtwork(
            _imageNumber, {"value": Web3.toWei(1, "ether")})
